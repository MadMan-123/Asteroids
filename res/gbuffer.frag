...
void main()
{
    // Albedo: texture * colour tint; if no texture, use colour directly
    vec4 albedoSample = texture(albedoTexture, tc);
    vec3 albedo;
    if (dot(albedoSample.rgb, albedoSample.rgb) > 0.001)
        albedo = (colour.r == 1.0 && colour.g == 1.0 && colour.b == 1.0) ? albedoSample.rgb : albedoSample.rgb * colour;
    else
        albedo = (colour.r == 1.0 && colour.g == 1.0 && colour.b == 1.0) ? vec3(1.0) : colour;
